" =============================================================================
" Filename: autoload/lightline/panda/components.vim
...
if g:lightline_panda_use_nerd_fonts_glyphs
  let s:mo_glyph = "\uf040 " " 
  let s:help_glyph = "\uf128" " 
...
if g:lightline_panda_use_powerline_glyphs
  let s:ro_glyph = "\ue0a2" " 
  let s:branch_glyph = "\ue0a0" " 
else
  let s:ro_glyph = 'RO'
...
function! lightline#panda#components#modified() abort
...
function! lightline#panda#components#readonly() abort
  return &buftype ==# 'terminal' ? '' :
...
function! lightline#panda#components#filepath() abort
  ...
  let ro_string = '' !=# lightline#panda#components#readonly() ? lightline#panda#components#readonly() . ' ' : ''
...
function! lightline#panda#components#filename() abort
  return (&buftype ==# 'terminal' ? has('nvim') ? b:term_title . ' (' . b:terminal_job_pid . ')' : '' :
        ...
        \ ('' !=# lightline#panda#components#modified() ? ' ' . lightline#panda#components#modified() : '')
endfunction

function! lightline#panda#components#fugitive() abort
...
function! lightline#panda#components#fileformat() abort
...
function! lightline#panda#components#charcode() abort
...
function! lightline#panda#components#ale_pre() abort
  let s:ale_linting = 1
  call lightline#update()
endfunction

function! lightline#panda#components#ale_post() abort
  let s:ale_linting = 0
  call lightline#update()
endfunction

function! lightline#panda#components#ale_error() abort
  return s:ale_string(0)
endfunction

function! lightline#panda#components#ale_warning() abort
  return s:ale_string(1)
endfunction

function! lightline#panda#components#ale_ok() abort
...
function! lightline#panda#components#lineinfo() abort
  return &filetype ==# 'denite' ? denite#get_status_linenr() :
        \ &filetype ==# 'tagbar' ? '' :
        \ printf('%3d:%-2d', line('.'), col('.'))
endfunction

function! lightline#panda#components#percent() abort
  if &filetype ==# 'tagbar'
    return ''
  endif
...
function! lightline#panda#components#currenttag() abort
  if !g:lightline_panda_tagbar_enable || &buftype ==# 'terminal' || &filetype =~# 'denite\|tagbar'
    return ''
  endif
...
function! lightline#panda#components#tagbar_status(current, sort, fname, ...) abort
  let g:lightline.fname = a:fname
  return lightline#statusline(0)
endfunction

function! lightline#panda#components#gitgutter_pre() abort
  let g:lightline_panda_gitgutter_context = get(g:, 'gitgutter_hook_context', {})
  call lightline#update()
endfunction

function! lightline#panda#components#gitgutter() abort
  if !g:lightline_panda_gitgutter_enable || winwidth(0) < 120
    return ''
  endif
  let ctx = get(g:, 'lightline_panda_gitgutter_context', {})
